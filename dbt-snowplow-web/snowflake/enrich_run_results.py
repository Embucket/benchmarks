#!/usr/bin/env python3
"""
Enrich run_results.json with actual row counts from Snowflake tables.

This script queries Snowflake to get the actual number of rows in each table
and updates the run_results.json file with accurate row counts. This is needed
because Snowflake's rows_affected returns 1 for CREATE TABLE statements instead
...
def get_table_row_count(conn, database, schema, table_name):
    """Query Snowflake to get the actual row count for a table."""
    cursor = conn.cursor()
    try:
        # Use COUNT(*) to get accurate row count
        # Don't quote identifiers - Snowflake will handle case sensitivity
...
def enrich_run_results(manifest_path, run_results_path, output_path):
    """
    Enrich run_results.json with actual row counts from Snowflake.
    ...
    for result in run_results['results']:
        ...
        # Get actual row count from Snowflake
...
def main():
    parser = argparse.ArgumentParser(description='Enrich run_results.json with actual row counts from Snowflake')
