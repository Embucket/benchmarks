...
def print_usage():
    ...
    print()
    print("Options:")
    print("  --rows N      Generate N rows per day (default: 1000)")
    print("  --gb SIZE     Generate SIZE GB of data per day")
    print()
    print("Examples:")
    print("  python gen_events.py --rows 10000")
    print("  python gen_events.py --gb 0.5")
...
def main():
    ...
    if use_size_mode:
        ...
        print(f"Target size per day: {target_size_gb} GB")
        print(f"Generating events for:")
        print(f"  Yesterday: {yesterday}")
        print(f"  Today: {today}")
        print()
        
        # Generate events for yesterday based on size
        print(f"Generating yesterday's events ({target_size_gb} GB)...")
        yesterday_events = generate_events_by_size('events_yesterday.csv', yesterday, target_size_gb)
        
        print()
        print(f"Generating today's combined events ({target_size_gb * 2} GB total)...")
        # For events_today.csv, we need to combine yesterday + today
        # Generate them together to reach the combined size
        today_events = generate_events_by_size('events_today.csv', today, target_size_gb * 2)
        ...
        print(f"Creating combined file ({num_events * 2:,} rows)...")
